...
contract CoffeeShopFactoryTest is Test {
    ...
    function test_Register() public {
        ...
        vm.expectEmit();
        emit CoffeeShopFactory.CoffeeShopCreated(
            cafe1Owner,
            cafe1ProxyAddresss,
            999999
        );
        ...
        vm.expectEmit();
        emit CoffeeShopFactory.UserIdAssigned(
            cafe1Owner,
            999999,
            expectedCafeUserId1
        );
        string memory cafeUserId1 = coffeeShopFactory.register(
            cafe1Owner,
            address(usdc),
            true,
            basisPoints
        ...
        string memory cafeUserId2 = coffeeShopFactory.register(
            cafe2Owner,
            address(usdc),
            false,
    ...
    function testFail_UserCannotDoubleRegister() public {
        ...
        vm.expectEmit();
        emit CoffeeShopFactory.OwnerRegistrationFailed(
            cafe1Owner,
            expectedFailureReason
        );
        string memory failureReason = coffeeShopFactory.register(
            cafe1Owner,
            address(usdc),
            true,
            basisPoints
